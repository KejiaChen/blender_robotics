...
def _scatter_tcp_points(points, radius=0.01, coll_name="TCP Points", material_name=None, template_name="TCP_Marker_Template"):
    template = _ensure_tcp_marker(template_name)
...
class SA_OT_load_a(Operator):
    ...
    def execute(self, context):
        ...
        scn = context.scene
        folder = scn.sa_folder + "_leader" or ""
...
class SA_OT_load_b(Operator):
    ...
    def execute(self, context):
        ...
        scn = context.scene
        folder = scn.sa_folder + "_follower" or ""
        path = os.path.join(folder, scn.da_common_filename) if scn.da_common_filename else ""
        if not path:
...
class SA_OT_tcp_visualize_a(Operator):
    ...
    def execute(self, context):
        ...
        path = os.path.join(folder+"leader", fname)
        try:
            ...
            n = _scatter_tcp_points(pts, radius=scn.tcp_radius, coll_name=f"TCP A ({fname})", material_name=scn.tcp_material, template_name="TCP_Marker_Template_1")
        except Exception as e:
            self.report({'ERROR'}, f"TCP A: {e}"); return {'CANCELLED'}
        self.report({'INFO'}, f"TCP A: Plotted {n} points")
        return {'FINISHED'}

class SA_OT_tcp_visualize_a2(Operator):
    bl_idname = "sa.tcp_visualize_a2"
    bl_label = "Visualize TCP A2"
    bl_options = {'REGISTER', 'UNDO'}
    def execute(self, context):
        scn = context.scene
        folder = scn.da_tcp2_filedir or ""
        fname  = scn.da_tcp2_filename or ""
        if not folder or not fname:
            self.report({'ERROR'}, "Set Arm A folder and TCP filename first"); return {'CANCELLED'}
        path = os.path.join(folder+"leader", fname)
        try:
            pts = _parse_tcp_positions(
                ...
                delim_mode=scn.da_delim,
                has_header=scn.sa_has_header,
                column_major=scn.tcp2_column_major,
                y_offset=0.281
            )
            # Downsample
            if scn.tcp2_step > 1:
                pts = pts[::scn.tcp2_step]
            n = _scatter_tcp_points(pts, radius=scn.tcp2_radius, coll_name=f"TCP2 A ({fname})", material_name=scn.tcp2_material, template_name="TCP_Marker_Template_2",)
        except Exception as e:
            self.report({'ERROR'}, f"TCP2 A: {e}"); return {'CANCELLED'}
        self.report({'INFO'}, f"TCP2 A: Plotted {n} points")
        return {'FINISHED'}
...
class SA_OT_tcp_visualize_b(Operator):
    ...
    def execute(self, context):
        ...
        path = os.path.join(folder+"follower", fname)
        try:
            pts = _parse_tcp_positions(
                path,
                delim_mode=scn.da_delim,
            ...
            if scn.tcp_step > 1:
                pts = pts[::scn.tcp_step]
            n = _scatter_tcp_points(pts, radius=scn.tcp_radius, coll_name=f"TCP B ({fname})", material_name=scn.tcp_material, template_name="TCP_Marker_Template_1")
        except Exception as e:
            self.report({'ERROR'}, f"TCP B: {e}"); return {'CANCELLED'}
        self.report({'INFO'}, f"TCP B: Plotted {n} points")
        return {'FINISHED'}

class SA_OT_tcp_visualize_b2(Operator):
    bl_idname = "sa.tcp_visualize_b2"
    bl_label = "Visualize TCP B2"
    bl_options = {'REGISTER', 'UNDO'}
    def execute(self, context):
        scn = context.scene
        folder = scn.da_tcp2_filedir or ""
        ...
        try:
            ...
            if scn.tcp2_step > 1:
                pts = pts[::scn.tcp2_step]
            n = _scatter_tcp_points(pts, radius=scn.tcp2_radius, coll_name=f"TCP2 B ({fname})", material_name=scn.tcp2_material, template_name="TCP_Marker_Template_2",)
        except Exception as e:
            self.report({'ERROR'}, f"TCP2 B: {e}"); return {'CANCELLED'}
        self.report({'INFO'}, f"TCP2 B: Plotted {n} points")
        return {'FINISHED'}
...
class SA_OT_tcp_clear_a2(Operator):
    bl_idname = "sa.tcp_clear_a2"
    bl_label  = "Clear TCP2 A"
    bl_options = {'REGISTER', 'UNDO'}
    def execute(self, context):
        n = _clear_tcp_by_prefix("TCP2 A (")
        self.report({'INFO'}, f"TCP2 A: cleared {n} collection(s)")
        return {'FINISHED'}
...
class SA_OT_tcp_clear_b2(Operator):
    bl_idname = "sa.tcp_clear_b2"
    bl_label  = "Clear TCP2 B"
    bl_options = {'REGISTER', 'UNDO'}
    def execute(self, context):
        n = _clear_tcp_by_prefix("TCP2 B (")
        self.report({'INFO'}, f"TCP2 B: cleared {n} collection(s)")
        return {'FINISHED'}
...
class VIEW3D_PT_dual_arm_traj(Panel):
    ...
    def draw(self, context):
        ...
        box.prop(scn, "sa_folder", text="Traj folder")  # DIR_PATH -> opens directory picker
        ...
        box.prop(scn, "da_tcp_filedir", text="TCP folder")
        ...
        row.operator("sa.tcp_clear_b", text="Clear TCP B", icon='TRASH')
         ...
         # --- TCP Block 2 ---
        box = layout.box()
        box.label(text="TCP Block 2", icon='SPHERE')
        box.prop(scn, "da_tcp2_filedir", text="TCP2 folder")
        box.prop(scn, "da_tcp2_filename", text="TCP2 filename")
        row = box.row(align=True)
        row.prop(scn, "tcp2_radius", text="Sphere radius")
        row.prop(scn, "tcp2_step",   text="Step (every Nth)")
        box.prop(scn, "tcp2_column_major", text="Matrix is Column-Major")
        box.prop(scn, "tcp2_material", text="Material")

        row = box.row(align=True)
        row.operator("sa.tcp_visualize_a2", text="Visualize TCP2 A", icon='DOT')
        row.operator("sa.tcp_visualize_b2", text="Visualize TCP2 B", icon='DOT')

        row = box.row(align=True)
        row.operator("sa.tcp_clear_a2", text="Clear TCP2 A", icon='TRASH')
        row.operator("sa.tcp_clear_b2", text="Clear TCP2 B", icon='TRASH')
...
classes = (
    ...
    SA_OT_tcp_visualize_a2, SA_OT_tcp_visualize_b2,
    SA_OT_tcp_clear_a, SA_OT_tcp_clear_b,
    SA_OT_tcp_clear_a2, SA_OT_tcp_clear_b2,
...
def register():
    ...
    bpy.types.Scene.sa_folder        = StringProperty(name="Arm A folder", default="/home/tp2/ws_humble/trajectories_leader/blender_render", subtype='DIR_PATH')
    ...
    # TCP Block 1 options
    bpy.types.Scene.da_tcp_filedir    = StringProperty(name="TCP folder", default="/home/tp2/ws_humble/trajectories_leader/blender_render", subtype='DIR_PATH')
    ...
    bpy.types.Scene.tcp_column_major  = BoolProperty(name="TCP Column-major 4x4", default=True)
    bpy.types.Scene.tcp_material      = StringProperty(name="TCP Material", default="trajectoryBlue")

    # TCP Block 2 options (NEW)
    bpy.types.Scene.da_tcp2_filedir    = StringProperty(name="TCP2 folder", default="/home/tp2/ws_humble/trajectories_follower/blender_render", subtype='DIR_PATH')
    bpy.types.Scene.da_tcp2_filename   = StringProperty(name="TCP2 filename", default="tcp2.txt")
    bpy.types.Scene.tcp2_radius        = FloatProperty(name="TCP2 sphere radius", default=0.005, min=0.0001, soft_max=0.2)
    bpy.types.Scene.tcp2_step          = IntProperty(name="TCP2 step", default=5, min=1)
    bpy.types.Scene.tcp2_column_major  = BoolProperty(name="TCP2 Column-major 4x4", default=True)
    bpy.types.Scene.tcp2_material      = StringProperty(name="TCP2 Material", default="trajectoryRed")
...
def unregister():
    ...
    del bpy.types.Scene.da_tcp_filedir
    ...
    del bpy.types.Scene.da_tcp2_filedir
    del bpy.types.Scene.da_tcp2_filename
    del bpy.types.Scene.tcp2_radius
    del bpy.types.Scene.tcp2_step
    del bpy.types.Scene.tcp2_column_major
    del bpy.types.Scene.tcp2_material
